...
def _font():
    global _FONT
    if _FONT is None: _FONT=pygame.font.SysFont("Arial",11,bold=True)
    return _FONT
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, enemy_type="ground", patrol_left=None, patrol_right=None):
        ...
        self.vel_x=-ENEMY_PATROL_SPEED; self.vel_y=0.0
        ...
        self.state="patrol"; self.alive=True; self.on_ground=False
    ...
    def die(self):
        self.alive=False; self.kill()

    def draw(self, surface, camera_offset_x):
        if not self.alive: return
        rx=self.rect.x-camera_offset_x; ry=self.rect.y
        ...
        if self.enemy_type=="ground":
            ...
            # Feet
            pygame.draw.rect(surface,(220,180,30),(rx+2,ry+h-6,8,6),border_radius=2)
            pygame.draw.rect(surface,(220,180,30),(rx+w-10,ry+h-6,8,6),border_radius=2)
            # Body block
            body=pygame.Rect(rx+1,ry+6+wob,w-2,h-10)
            pygame.draw.rect(surface,(60,170,75),body,border_radius=4)
            pygame.draw.rect(surface,(95,210,100),(body.x+2,body.y+2,body.width-4,6),border_radius=2)
            pygame.draw.rect(surface,(35,110,45),body,2,border_radius=4)
            # Yellow belly stripe
            pygame.draw.rect(surface,(245,210,40),(body.x+4,body.bottom-7,body.width-8,5))
            # Spikes on top
            for i in range(3):
                sx=body.x+6+i*((body.width-12)//2)
                pygame.draw.polygon(surface,(245,210,40),[(sx,body.y),(sx+4,body.y-5),(sx+8,body.y)])
            # Eyes
            ey=body.y+10
            ...
            pygame.draw.ellipse(surface,(245,210,40),(rx+w-10,ry+h-7,12,8))
            # Red dome body
            pygame.draw.ellipse(surface,(195,40,40),(rx,ry+wob,w,h-2))
            pygame.draw.ellipse(surface,(235,90,90),(rx+4,ry+2+wob,w-8,h//2))
            pygame.draw.ellipse(surface,(120,20,20),(rx,ry+wob,w,h-2),2)
            # Highlight stripe
            pygame.draw.line(surface,(255,255,255),(rx+w//4,ry+5+wob),(rx+w//2,ry+3+wob),2)
            # Big cartoon eyes
            ey=cy-2+wob
            ...
            # Wings (flap)
            pygame.draw.polygon(surface,(220,170,255),
                [(rx-6,cy-flap),(rx+4,ry-2),(rx+10,cy)])
            pygame.draw.polygon(surface,(220,170,255),
                [(rx+w+6,cy-flap),(rx+w-4,ry-2),(rx+w-10,cy)])
            pygame.draw.polygon(surface,(160,110,210),
                [(rx-6,cy-flap),(rx+4,ry-2),(rx+10,cy)],1)
            pygame.draw.polygon(surface,(160,110,210),
                [(rx+w+6,cy-flap),(rx+w-4,ry-2),(rx+w-10,cy)],1)
            # Round purple body (ghost shape)
            pygame.draw.ellipse(surface,(140,90,200),(rx,ry,w,h+4))
            pygame.draw.ellipse(surface,(180,130,230),(rx+3,ry+2,w-6,h//2))
            pygame.draw.ellipse(surface,(80,40,130),(rx,ry,w,h+4),2)
            # Eyes (yellow ovals)
            ey=ry+h//3
            pygame.draw.ellipse(surface,(0,0,0),(rx+w//4-3,ey-4,8,10))
            pygame.draw.ellipse(surface,(0,0,0),(rx+3*w//4-5,ey-4,8,10))
            pygame.draw.ellipse(surface,(255,225,60),(rx+w//4-1,ey-2,4,6))
            pygame.draw.ellipse(surface,(255,225,60),(rx+3*w//4-3,ey-2,4,6))
