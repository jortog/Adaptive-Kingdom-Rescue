...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y, game_state):
        ...
        self.is_dashing=False; self.dash_timer=0.0; self.dash_cooldown=0.0
        self.jump_held=False; self.jump_held_timer=0.0; self.JUMP_HOLD_MAX=0.18
        self.can_double_jump=False; self.has_double_jumped=False
        self._jump_buffer_timer=0.0; self._JUMP_BUFFER_TIME=0.12
        self._coyote_timer=0.0; self._COYOTE_TIME=0.10; self._was_on_ground=False
        self.size_level=1; self.has_flower=False; self.star_timer=0.0; self.shield_count=0
        self._action_record_timer=0.0; self._ACTION_RECORD_INTERVAL=0.2
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            ...
            if event.key in (pygame.K_z, pygame.K_SPACE):
                ...
                self.jump_held=True; self.jump_held_timer=0.0
    ...
    def update(self, dt, platforms, game_state):
        ...
        if self._was_on_ground and not self.on_ground: self._coyote_timer=self._COYOTE_TIME
        if self.on_ground: self._coyote_timer=0.0
        else: self._coyote_timer=max(0.0,self._coyote_timer-dt)
        can_jump=self.on_ground or self._coyote_timer>0
        if self._jump_buffer_timer>0 and can_jump:
            self._do_jump(); self._jump_buffer_timer=0.0; self._coyote_timer=0.0
        elif self._jump_buffer_timer>0 and self.can_double_jump and not self.has_double_jumped:
            self._do_jump(); self.has_double_jumped=True; self._jump_buffer_timer=0.0
        ...
        if abs(self.vel_x)>10 and self.on_ground: self._walk_phase+=dt*12
        else: self._walk_phase=0.0
        self._was_on_ground=self.on_ground

    def _do_jump(self):
        self.vel_y=PLAYER_JUMP_VELOCITY; self.on_ground=False
        self.game_state.record_action(ACTION_JUMP)
        if self.audio: self.audio.play_jump()

    def _try_dash(self):
        if self.dash_cooldown<=0 and not self.is_dashing:
            self.is_dashing=True; self.dash_timer=PLAYER_DASH_DURATION
            self.dash_cooldown=PLAYER_DASH_COOLDOWN
            self.game_state.record_action(ACTION_DASH)

    def _try_attack(self):
        if self.has_flower:
            self.game_state.record_action(ACTION_ATTACK); return True
        return False

    def _handle_horizontal(self, keys, dt):
        if self.is_dashing:
            self.vel_x=PLAYER_DASH_SPEED*(1 if self.facing_right else -1); return
        if keys[pygame.K_LEFT] or keys[pygame.K_a]:
            self.vel_x=-PLAYER_RUN_SPEED; self.facing_right=False
        elif keys[pygame.K_RIGHT] or keys[pygame.K_d]:
            self.vel_x=PLAYER_RUN_SPEED; self.facing_right=True
        else: self.vel_x=0
    ...
    def _apply_gravity(self, dt):
        if not self.on_ground:
            self.vel_y+=GRAVITY*dt; self.vel_y=min(self.vel_y,1200)
    ...
    def _move_and_collide(self, platforms, dt):
        ...
        for p in platforms:
            if self.rect.colliderect(p):
                if self.vel_y>0:
                    self.rect.bottom=p.top; self.on_ground=True; self.vel_y=0
                elif self.vel_y<0:
    ...
    def _update_timers(self, dt):
        ...
        if self.star_timer>0: self.star_timer-=dt
    ...
    def take_damage(self):
        ...
        if self.shield_count>0: self.shield_count-=1; return False
        if self.size_level>1: self.size_level-=1; self.has_flower=False; return False
        return True
    ...
    def draw(self, surface, camera_offset_x):
        ...
        pygame.draw.rect(surface, steel_d, (cx-9, ry+h-12+leg_off, 7, 12), border_radius=2)
        pygame.draw.rect(surface, steel_d, (cx+2, ry+h-12-leg_off, 7, 12), border_radius=2)
        pygame.draw.rect(surface, steel_m, (cx-9, ry+h-12+leg_off, 7, 4), border_radius=2)
        pygame.draw.rect(surface, steel_m, (cx+2, ry+h-12-leg_off, 7, 4), border_radius=2)

        # Body / cuirass
        body = pygame.Rect(cx-11, ry+h//3, 22, h//2)
        pygame.draw.rect(surface, steel_m, body, border_radius=4)
        pygame.draw.rect(surface, steel_l, (body.x+2, body.y+2, body.width-4, 6), border_radius=3)
        ...
        pygame.draw.line(surface, steel_d, (sh_x, ry+h//2-4),(sh_x, ry+h//2+13),1)

        # Sword (front arm)
        sw_x = cx + 13*flip
        if self.has_flower or self.is_dashing:
            pygame.draw.line(surface, blade,  (sw_x, ry+h//2), (sw_x+10*flip, ry-6), 3)
            pygame.draw.line(surface, blade_c,(sw_x, ry+h//2), (sw_x+10*flip, ry-6), 1)
        else:
            pygame.draw.line(surface, steel_l, (sw_x, ry+h//2+6),(sw_x, ry+h//4), 3)
        pygame.draw.line(surface, leather, (sw_x-3*flip, ry+h//2+3),(sw_x+3*flip, ry+h//2+3), 3)

        # Helmet
        hy = ry + 2
        helm = pygame.Rect(cx-9, hy, 18, h//3+2)
        pygame.draw.rect(surface, steel_m, helm, border_radius=5)
        pygame.draw.rect(surface, steel_l, (helm.x+2, helm.y+1, helm.width-4, 4), border_radius=3)
        # Visor slit
        pygame.draw.rect(surface, (20,24,30), (helm.x+3, helm.centery, helm.width-6, 3))
        pygame.draw.rect(surface, (20,24,30), (cx-1, helm.y+5, 2, helm.height-9))
        # Horns
        pygame.draw.polygon(surface, horn, [(helm.x-1,hy+4),(helm.x-7,hy-4),(helm.x+2,hy+1)])
        pygame.draw.polygon(surface, horn, [(helm.right+1,hy+4),(helm.right+7,hy-4),(helm.right-2,hy+1)])

        # Shield aura
        if self.shield_count>0:
            pygame.draw.rect(surface,(80,200,255),(rx-4,ry-4,w+8,h+8),2,border_radius=6)
