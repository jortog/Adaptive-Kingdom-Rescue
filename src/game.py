...
class Game:
    ...
    def _build_ppo_state(self, jump_freq=None, run_freq=None):
        ...
        alive_count = sum(
            1 for e in self.enemies if getattr(e, 'is_alive', True))
    ...
    def _record_ppo_transition(self, state, action, reward, done=False):
        if state is not None and hasattr(self.ai_ensemble, 'ppo') and self.ai_ensemble.ppo:
            next_state = self._build_ppo_state()
            if next_state is not None:
                self.ai_ensemble.ppo.observe(
                    state, action, reward, next_state, done)

    def _lose_life(self, ppo_state, ppo_action, frame_reward):
        """Handle life loss and reload level or game over."""
        self.gs.lives -= 1
        frame_reward += 0.75
    ...
    def update_game(self, dt):
        ...
        elevation_tiles = max(
            0.0, (ground_top - self.player.rect.bottom) / TILE_SIZE)
        if elevation_tiles >= 1.5:
            self._elevated_time = min(
                self._elevated_time + dt, ELEVATION_CAMP_FULL + 1.0)
        else:
        ...
        ppo_action = getattr(self.ai_ensemble, '_ppo_action', 0)
        frame_reward = dt
        player_jumped = self.player.vel_y < -300 and not self.player.on_ground
        spawn_requested = False

        ordered = sorted(
            (e for e in self.enemies if getattr(e, 'is_alive', True)),
    ...
    def draw_game(self):
        ...
        self.hud.draw(self.screen, self.gs, self.time_remaining,
                      self._max_level_progress, self.player)
        ...
        if event.key == pygame.K_UP:
            ...
            if c == 0:
                ...
                if event.key == pygame.K_RIGHT:
                    ...
                    self.selected_level = (
                        self.selected_level - 1) % NUM_LEVELS
            elif c == 3 and event.key == pygame.K_RETURN:
