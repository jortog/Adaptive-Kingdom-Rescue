...
_FIREFLIES = [
    (random.randint(50, SCREEN_WIDTH-50),
     ...
     random.random()*6.28, 0.6+random.random()*1.6)
    for _ in range(18)
]
random.seed(33)
_TREES = [(tx, random.randint(55,115))
...
def _build_castle():
    ...
    for tx,th in _TREES:
        ...
        pygame.draw.polygon(surf,(10,16,26),[(tx+7,by-th-32),(tx-8,by-th+12),(tx+22,by-th+12)])
    return surf
...
def _draw_stars(surface,t):
    for sx,sy,phase,size in _STARS:
        ...
        col    = (int(C_STAR[0]*bright),int(C_STAR[1]*bright),min(255,int(C_STAR[2]*bright)))
...
def _center_outline(surface,text,font,col,y,n=2,sc=C_BLACK):
    ...
    _outline(surface,text,font,col,SCREEN_WIDTH//2-img.get_width()//2,y,n,sc)
...
class ScreenManager:
    ...
    def draw_main_menu(self,cursor=0):
        ...
        M = 58
        _outline(self.surface,"ADAPTIVE",self.fp_title,C_GOLD,M,82,n=2,sc=(75,48,0))
        _outline(self.surface,"KINGDOM RESCUE",self.fp_title,C_GOLD,M,130,n=2,sc=(75,48,0))
        ...
        for i,item in enumerate(ITEMS):
            ...
            if sel:
                ...
                _outline(self.surface,item,self.fp_large,C_GOLD,M,iy,n=2,sc=(75,48,0))
            else:
        ...
        pygame.draw.line(self.surface,(45,40,30),(M,SCREEN_HEIGHT-52),(div_x-16,SCREEN_HEIGHT-52),1)

        tags = [("Decision Tree",(85,215,85)),("  +  RNN/LSTM",(85,155,255)),("  +  PPO",(255,160,65))]
        fx = M
        for ltxt,lcol in tags:
            img = self.fp_small.render(ltxt,True,lcol)
            self.surface.blit(self.fp_small.render(ltxt,True,C_BLACK),(fx+1,SCREEN_HEIGHT-44+1))
    ...
    def draw_settings(self, cursor=0, settings=None,
                      selected_level=0, num_levels=3, from_game=False):
        if settings is None:
            settings = {}
        t = self._t
        ...
        _outline(self.surface,"SETTINGS",self.fp_large,C_GOLD,
        ...
        for i,(label,value) in enumerate(options):
            ...
            if sel:
                ...
                self.surface.blit(self.fp_med.render(">",True,C_GOLD),(px+20,row_y+2))
        ...
        if cursor==3:
            ...
            for ci,(act,key) in enumerate(ctrls):
                self.surface.blit(self.fs_tiny.render(act,True,(110,104,88)),(px+46, cy2+ci*20))
                self.surface.blit(self.fs_tiny.render(key,True,(165,158,132)),(px+175,cy2+ci*20))
    ...
    def draw_pause(self,cursor=0):
        ...
        _outline(self.surface,"PAUSED",self.fp_large,C_GOLD,
                 px+pw//2-timg.get_width()//2,py+18,n=2,sc=(75,48,0))
        pygame.draw.line(self.surface,(50,45,32),(px+20,py+56),(px+pw-20,py+56),1)
        ...
        for i,item in enumerate(["RESUME","SETTINGS","BACK TO MENU"]):
            ...
            self.surface.blit(self.fp_med.render(item,True,C_GOLD if sel else C_DIM),(px+46,iy2))
    ...
    def draw_win_screen(self,score):
        ...
        for i in range(14):
            ...
            pygame.draw.circle(self.surface,C_GOLD,
